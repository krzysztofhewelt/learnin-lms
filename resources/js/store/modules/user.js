...
const user = {
    ...
    actions: {
        async getUserDetails({ commit }, userId = "") {
            ...
            const userUrl =
                userId === "" ? "/profile" : "/users/show/" + userId;
            ...
            await axios.get(userUrl).then((response) => {
                ...
                if (response.data.teacher !== null)
                    commit("setTeacher", response.data.teacher);
        ...
        async getUserCourses({ dispatch, commit }, userId = "") {
            ...
            const coursesUrl =
                userId === "" ? "/courses" : "/users/courses/" + userId;
        ...
        async getUserTasks({ dispatch, commit }, userId = "") {
            ...
            const tasksUrl =
                userId === "" ? "/tasks" : "/user/" + userId + "/tasks";
    ...
    getters: {
        ...
        isUserAdmin(state) {
            return (
                state.user.account_role &&
                state.user.account_role.name === "admin"
            );
        },
        ...
        isUserStudent(state) {
            return (
                state.user.account_role &&
                state.user.account_role.name === "student"
            );
        },
        ...
        isUserTeacher(state) {
            return (
                state.user.account_role &&
                state.user.account_role.name === "teacher"
            );
        },
            ...
            return (
                ...
                state.courses.filter(
                    (course) =>
                        ...
                        (dayjs() <= dayjs(course.available_to) ||
                            course.available_to === null)
                )
            );
        },
        ...
        getUpcomingCourses(state) {
            return (
                state.courses &&
                state.courses.filter(
                    (course) => dayjs() < dayjs(course.available_from)
                )
            );
        },
            ...
            return (
                ...
                state.tasks.filter(
                    (task) =>
                        ...
                        (dayjs() <= dayjs(task.available_to) ||
                            task.available_to === null)
                )
            );
        },
        ...
        getMarkedTasks(state) {
            return (
                state.tasks &&
                state.tasks.filter((task) => task.user_marks.length > 0)
            );
        },
        ...
        getUpcomingTasks(state) {
            return (
                state.tasks &&
                state.tasks.filter(
                    (task) => dayjs() < dayjs(task.available_from)
                )
            );
        },
        ...
        getTeachersInCourse: (state) => (course) => {
            return (
                course.users &&
                course.users.filter((user) => user.account_role === "teacher")
            );
        },
    },
};
