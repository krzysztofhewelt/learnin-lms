...
const user = {
    ...
    actions:{
        ...
        async createNewUserOrEdit({ commit }, { user, student, teacher }) {
            ...
            const postUrl = (!user.id) ? '/users/create-or-edit/0' : '/users/create-or-edit/' + user.id

            return await axios.post(postUrl, dataUserObject)
                .then(
                    (response) => {
                        commit('clearValidationErrors')
                        commit('loading', false)
        ...
        async getCategoryTasks({ commit }, courseId) {
            ...
            await axios.get('/tasks/show/category/' + courseId)
                .then(
                    response => {
                        commit('setTasks', response.data)
                    }
        ...
        async updateTeacherInformations({ commit }, { userId, teacher })
        {
            ...
            await axios.post('/users/update-teacher/' + userId, teacher)
                .then(
                    () => {
                        commit('clearValidationErrors')
    ...
    getters: {
        ...
        getEndedCourses(state) {
            return state.courses
                && state.courses.filter(course => dayjs() > dayjs(course.available_to)
        ...
        getActiveTasks(state) {
            return state.tasks
                && state.tasks.filter(task => dayjs() >= dayjs(task.available_from)
                    && (dayjs() <= dayjs(task.available_to) || task.available_to === null))
        },

        getExpiredTasks(state) {
            return state.tasks
                && state.tasks.filter(task => (dayjs() > dayjs(task.available_to)
