...
const task = {
    ...
    actions: {
        ...
        async showTask({ commit }, taskId) {
            ...
            await axios.get('/tasks/show/' + taskId)
                .then(
                    response => {
                        commit('setTask', {
        ...
        async createOrEditTask({ commit }, task) {
            ...
            if(task.available_to === '')
                delete task.available_to
        ...
        async deleteStudentFile({ commit }, fileId) {
            ...
            await axios.post('/delete-resource/' + fileId, {
                file_type: 'student_upload'
            })
                .then(
                    () => {
                        commit('setStudentFiles', state.studentFiles.filter(file => file.id !== fileId))
                    }
                )
        ...
        async deleteTaskRefFile({ commit }, fileId) {
            ...
            await axios.post('/delete-resource/' + fileId, {
                ...
                .then(
                    () => {
                        commit('setTaskRefFiles', state.taskRefFiles.filter(file => file.id !== fileId))
                    }
                )
        ...
        async getStudentFilesInTask({ dispatch, commit }, taskId) {
            ...
            await axios.get('/tasks/students_uploads/' + taskId)
                .then(
                    response => {
    ...
    getters: {
        ...
        studentCanUploadFiles(state) {
            return dayjs() >= dayjs(state.task.available_from) && (state.task.available_to === null || dayjs() <= dayjs(state.task.available_to))
        },
        ...
        isEnded(state) {
            return state.task.available_to !== null
                && dayjs() > dayjs(state.task.available_to)
        }
    }
}
