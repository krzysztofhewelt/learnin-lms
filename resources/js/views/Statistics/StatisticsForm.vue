<template>
    <div class="relative mb-6 mt-6 px-6 py-6 flex w-full min-w-0 flex-col break-words rounded-lg bg-white shadow-xl">
        <div class="grid grid-cols-1 lg:grid-cols-3">
            <div class="mb-3 lg:col-start-1">
                <span class="font-bold text-2xl">{{ $t('statistics.select_type') }}:</span>
                <Multiselect v-model="statType"
            ...
            <div class="mb-3 lg:col-start-1">
                <span class="font-bold text-2xl">{{ $t('statistics.select_course') }}:</span>
                <Multiselect v-model="course"
                             :options="courses || []"
                             :loading="loading"
                             :placeholder="$t('general.select_one')"
                             label="name"
            ...
            <div class="mb-3 lg:col-start-1">
                <span class="font-bold text-2xl">{{ $t('statistics.select_category') }}:</span>
            ...
            <div class="mb-3 lg:col-start-1" v-if="statType.id === 'fromTask'">
                <span class="font-bold text-2xl">{{ $t('statistics.select_task') }}:</span>
                <Multiselect v-model="task"
                             :options="tasks"
                             :loading="loading"
                             :placeholder="$t('general.select_one')"
                             label="name"
        ...
        <div v-if="reportGenerated" class="border-t-[1px]">
            ...
            <BaseTable :title="statType.name"
                       :columns="resultColumns"
                       :loading="loading">
                <tr v-if="resultData.length === 0">
                    <BaseRow :colspan="resultColumns.length"
                             class="text-center font-bold text-xl">
                        {{ $t('task.no_tasks') }}
                    </BaseRow>
                </tr>
                <tr v-for="student in resultData">
                    <BaseRow>{{ student.surname }}</BaseRow>
                    <BaseRow>{{ student.name }}</BaseRow>
                    <BaseRow>{{ student.identification_number }}</BaseRow>
                    <BaseRow>{{ student.points || '0.00' }}</BaseRow>
                    <BaseRow>{{ student.mark || '0.00' }}</BaseRow>
                </tr>
            </BaseTable>
        </div>
...
export default {
    ...
    computed: {
        ...
        statTypeSelectedCorrectly() {
            return (this.statType && this.statType.id === 'fromCourseCategory' && this.course && this.category)
                || (this.statType && this.statType.id === 'fromTask' && this.course && this.category && this.task)
        }
    },
    ...
    methods: {
        ...
        async generateReport() {
            ...
            if(this.statType.id === 'fromTask') {
                await axios.get('/statistics/task/' + this.task.id)
                    .then(
                        response => {
            ...
            else if(this.statType.id === 'fromCourseCategory')
            {
                ...
                await axios.get('/statistics/course/' + this.course.id + '/' + this.category.id)
                    .then(
                        response => {
