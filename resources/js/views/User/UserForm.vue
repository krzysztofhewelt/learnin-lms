<template>
    <div class="flex relative w-full h-fit flex-col break-words rounded-lg bg-white shadow-xl">
        ...
        <form @submit.prevent="handleSubmit">
            <div class="p-6">
                <h1 class="my-auto block text-xl font-bold mb-4">
                    {{ (this.$route.name === 'UsersEdit') ? $t('user.editing_user') : $t('user.creating_user') }}
                </h1>

                <div class="grid grid-cols-4 gap-x-4 gap-y-2">
                    <div class="col-span-4 lg:col-span-1">
                        <label for="account_role">{{ $t('user.account_role') }}</label>
                        <Multiselect id="account_role"
                                     ...
                                     label="label"
                                     track-by="name" />
                        <span class="block text-red-400 font-bold mt-1" v-for="error in validationErrors.account_role">
                            {{ error }}
                        </span>
                    </div>
                    ...
                    <div class="col-span-4 lg:col-span-1 lg:col-start-1">
                        <BasePassword
                            ...
                            :placeholder="($route.name === 'UsersEdit') ? $t('user.empty_password_placeholder') : ''"
                    ...
                    <div class="col-span-4 mb-2 lg:col-span-1 lg:col-start-1">
                        ...
                        <Multiselect id="locale"
                                     ...
                                     label="label"
                                     track-by="name" />
                        <span class="block text-red-400 font-bold mt-1" v-for="error in validationErrors.locale">
                            {{ error }}
                        </span>
                    </div>
                    <div class="col-span-4 lg:col-span-1 lg:col-start-1">
                    ...
                    <div class="col-span-4 text-red-400 font-bold text-2xl mt-1" v-if="validationErrors.student">
                        {{ $t('user.validation_student') }}
                    </div>

                    <template v-if="user.account_role && user.account_role.name === 'student'">
                        <div class="col-span-4 border-t mt-4 pt-4 lg:col-span-1 lg:col-start-1">
                            <h1 class="font-bold text-2xl">
                                {{ $t('user.student_fields_of_study') }}
                            </h1>
                            <button class="normal_btn" @click.prevent="addFieldOfStudy">{{ $t('user.add_field_of_study') }}</button>
                        </div>
                        <template v-for="(student_info, index) in student">
                            ...
                            <div class="col-span-4 lg:col-span-1 lg:col-start-1">
                                <BaseInputGroup
                                    ...
                                    :validation-errors="validationErrors['student.' + index + '.field_of_study']"
                                />
                            </div>
                            <div class="col-span-4 lg:col-span-1">
                                <BaseInputGroup
                                    ...
                                    :validation-errors="validationErrors['student.' + index + '.semester']"
                                />
                            </div>
                            <div class="col-span-4 lg:col-span-1">
                                <BaseInputGroup
                                    ...
                                    :validation-errors="validationErrors['student.' + index + '.year_of_study']"
                                />
                            </div>
                            <div class="col-span-4 lg:col-span-1">
                                <BaseInputGroup
                                    ...
                                    :validation-errors="validationErrors['student.' + index + '.mode_of_study']"
                                />
                            </div>
                            <div class="col-span-4 lg:col-span-1 lg:col-start-1">
                                <button class="delete_btn" @click.prevent="removeFieldOfStudy(index)">{{ $t('user.remove_field_of_study') }}</button>
                            </div>
                        </template>
                    </template>
            ...
            <div class="px-4 py-3 bg-gray-100 text-right sm:px-6 rounded-b-lg">
...
export default {
    name: "UserForm",
    components: {BaseCheckbox, BasePassword, ButtonSubmit, BaseInputGroup, LoadingScreen, Multiselect},
    ...
    methods: {
        ...
        handleSubmit() {
            ...
            this.createNewUserOrEdit({ user: this.user, student: this.student, teacher: this.teacher })
                .then(
                    (response) => {
                        toast.success(this.$t('general.saved_successfully'))
                        router.push({name:'UsersView', params:{id: response.data.user_ID}})
                    }
                )
                .catch((error) => {
