...
class UploadController extends Controller
{
    ...
    public function uploadResources(\Illuminate\Http\Request $request, int $resourceId)  : JsonResponse
    {
        $fileType = $request->file_type;
        $files = $request->file('files');

        $this->authorize('upload-files', [$fileType, $this->getCourseForResource($fileType, $resourceId)]);
        ...
        if($request->isMethod('POST'))
        {
            foreach($files as $file)
            {
                ...
                $fileToUpload = $this->getFileByName($file->getClientOriginalName(), $fileType, $resourceId);
                ...
                else if($fileType == 'task_ref')
                {
                    $fileToUpload->task_ID = $resourceId;
                }
                else if($fileType == 'course_file')
                {
                    $fileToUpload->course_ID = $resourceId;
                }

                // fix for .exe files and nginx.:
                $filenameHashed = pathinfo($file->hashName(), PATHINFO_FILENAME) . '.' . $file->getClientOriginalExtension();
                $fileToUpload->filename = $filenameHashed;
                $fileToUpload->filename_original = $file->getClientOriginalName();

                $fileDir = $this->getFileDir($fileType, $this->getCourseIdForResource($fileType, $resourceId));
    ...
    private function getInstanceOfNewFileObject(string $fileType) : StudentFile|TaskFile|CourseFile|null
    {
    ...
    private function getCourseForResource(string $fileType, int $resourceId)
    {
        ...
        {
            return $this->taskModel->getTask($resourceId)->course()->first();
        }
        else if($fileType == 'course_file')
        {
    ...
    private function getFileById(string $fileType, int $resourceId) : StudentFile|TaskFile|CourseFile|null
    {
        return match($fileType) {
    ...
    private function getFileByName(string $filenameOriginal, string $fileType, int $resourceId) : StudentFile|TaskFile|CourseFile|null
    {
        return match($fileType) {
            'student_upload' => $this->studentFileModel->getFileForTaskAndUser($resourceId, Auth::id(), $filenameOriginal),
            'task_ref' => $this->taskFileModel->getFileForTask($resourceId, $filenameOriginal),
            'course_file' => $this->courseFileModel->getFileForCourse($resourceId, $filenameOriginal)
        };
    }
