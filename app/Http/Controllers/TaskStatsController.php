...
class TaskStatsController extends Controller
{
    ...
    public function computeAvgMarksForCourseCategory(int $courseId, int $courseCategoryId): JsonResponse
    {
        ...
        if($courseCategory->course_ID != $courseId)
            return response()->json(['error' => 'This course category does not belongs to this course']);

        $this->authorize('view-marks-statistics', $courseCategory->course);

        return response()->json($this->taskMarkModel->getMarksPointsAvgForCourseCategory($courseCategoryId, $courseId));
    }
