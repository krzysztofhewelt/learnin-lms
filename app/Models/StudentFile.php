...
class StudentFile extends Model
{
     ...
     * Business logic
     */
    public function getFileForTaskAndUser(int $taskId, int $userId, string $filenameOriginal) : ?StudentFile
    {
        return $this->where(['task_ID' => $taskId, 'filename_original' => $filenameOriginal, 'user_ID' => $userId])
                    ->first();
    }
    ...
    {
        return $this->where(['user_ID' => $userId, 'task_ID' => $taskId])
                    ->get();
    }
    ...
    public function getStudentsUploadedFilesForTask(int $taskId)
    {
        ...
        if($courseStudentsIds == null) $courseStudentsIds = 'null';

        $studentFilesForTask = DB::select('SELECT users.id AS user_ID,name,surname,users_tasks_uploads.id AS file_ID,filename,filename_original,file_size,file_size_unit,created_at,updated_at
                           FROM users
                           LEFT JOIN users_tasks_uploads
                           ON users.id = users_tasks_uploads.user_ID AND task_ID=?
                           WHERE users.id IN ('.$courseStudentsIds.')
                           ORDER BY surname;', [$taskId]
        );
        ...
        $studentFiles = array_reduce($studentFilesForTask, function($accumulator, $item){
            $index = $item['user_ID'];
