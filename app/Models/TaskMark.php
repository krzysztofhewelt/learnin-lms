...
class TaskMark extends Model
{
    ...
    {
        return $this->where(['user_ID' => Auth::id(), 'task_ID' => $taskId])
                    ->first();
    }
    ...
    public function getMarksPointsAvgForCourseCategory(int $courseCategoryId, int $courseId) : array
    {
        ...
        if($tasksIds == null) $tasksIds = 'null';
        ...
        return DB::select('SELECT users.id,name,surname,identification_number,COALESCE(SUM(mark)/?) AS \'mark\', COALESCE(SUM(points)/?) AS \'points\'
                          FROM users
    ...
    public function getMarksForTaskAvg(int $taskId) : array
    {
        ...
        $taskAvg = DB::select(
            'SELECT COALESCE(AVG(mark), \'n/a\') AS \'avg(mark)\', COALESCE(AVG(points), \'n/a\') AS \'avg(points)\' FROM tasks_marks WHERE task_ID = ?;', [$taskId]
        );
