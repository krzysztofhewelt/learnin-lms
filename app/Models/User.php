...
class User extends Authenticatable implements JWTSubject
{
    ...
    {
        return $this->belongsToMany(Task::class, 'tasks_marks', 'user_ID', 'task_ID')
    ...
    public function getTeachers($allTeachers) : LengthAwarePaginator|Collection
    {
        return match($allTeachers) {
            'all' => $this->where('account_role', 'teacher')->with('teacher')->get(),
            default => $this->where('account_role', 'teacher')->with('teacher')->paginate(15)
        };
    }
    ...
    {
        return $this->with('teacher', 'student')
            ->find($userId);
    }
