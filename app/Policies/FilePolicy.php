...
class FilePolicy
{
    ...
    public function deleteFiles(User $user, string $fileType, Course $course, CourseFile|TaskFile|StudentFile $file): bool
    {
        ...
        else
        {
            return ($user->isTeacher()
                    && $course->isTeacherOfCourse($user->id, $course->id))
                    || $user->isAdmin();
        }
    }

    public function downloadFiles(User $user, string $fileType, Course $course, CourseFile|TaskFile|StudentFile|null $file): bool
    {
        if($fileType == 'student_upload')
        {
            return ($user->isStudent()
        ...
        {
            return $course->isUserBelongsToCourse($user->id, $course->id)
                || $user->isAdmin();
        }
    }
